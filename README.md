...
This submission replaces the starter's intent-only model plus constant-velocity
trajectory with a two-part tabular model:

- **Intent:** an XGBoost classifier trained on the starter's compact 20-feature
  motion/ego/scene representation. This calibrated better on Dev than the
  larger trajectory feature set, which matters because intent is scored with
  log-loss.
- **Trajectory:** eight XGBoost regressors, one `dx`/`dy` residual for each
  future horizon. Each regressor starts from the constant-velocity center
  extrapolation and learns a correction from the full 16-frame bbox history,
  recent velocity/acceleration, bbox scale change, ego speed/yaw, frame timing,
  and scene metadata. The current bbox width/height is carried forward because
  the grader scores center ADE.

Final local results:

| Run | Composite | Intent term | Traj term | BCE | ADE |
|---|---:|---:|---:|---:|---:|
| `python grade.py` 5k Dev sample | **0.7203** | 0.844 | 0.597 | 0.2100 | 29.7 px |
...
this environment, so the improvement comes mainly from reducing trajectory ADE.
...
External data / pretrained weights: none. The model uses only the provided
`data/train.parquet` labels for fitting and `data/dev.parquet` for validation.

If I kept iterating, the next experiments would be a small GRU/temporal MLP
trained jointly on BCE plus Huber center loss, and a post-hoc probability
calibration pass for intent.
